...
export function AddingPhase({state, myId, isHost, send, myPlayer}: PhaseProps) {
	...
	const nextPhaseName = state.max_vetoes === 0 ? "Voting" : "Vetoing";
					...
					<button
						...
						onClick={() => {
							if (readyCount < totalCount && !window.confirm(
								`Only ${readyCount}/${totalCount} players are ready. Advance to ${nextPhaseName} anyway?`
							)) return;
							send({ty: "advance_phase"});
						}}
					>
						Advance to {nextPhaseName} ({readyCount}/{totalCount} ready)
					</button>
				</section>
			)}
		</>
